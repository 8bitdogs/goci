...
type Push struct {
	...
	HeadCommit struct {
		...
		Timestamp        time.Time `json:"timestamp"`
		URL              string    `json:"url"`
		HeadCommitAuthor struct {
			Name     string    `json:"name"`
			Email    string    `json:"email"`
			Date     time.Time `json:"date"`
			Username string    `json:"username"`
		} `json:"author"`
		HeadCommitCommitter struct {
			Name     string    `json:"name"`
			Email    string    `json:"email"`
			Date     time.Time `json:"date"`
			Username string    `json:"username"`
		} `json:"committer"`
		HeadCommitAdded    []any    `json:"added"`
		HeadCommitRemoved  []any    `json:"removed"`
		HeadCommitModified []string `json:"modified"`
	} `json:"head_commit"`
}

func (p *Push) TargetBranch() string {
	return strings.TrimPrefix(p.Ref, "refs/heads/")
}

func (p *Push) FullName() string {
	return p.Repository.FullName
}

func (p *Push) Sha() string {
	return p.After
}
